...
class FastText {
 ...
 protected:
  ...
  std::vector<int64_t> getTargetCounts() const; // 获取 target 的频率
  std::shared_ptr<Loss> createLoss(std::shared_ptr<Matrix>& output); // 根据 args_->loss 来生成 loss_
  void supervised(
      Model::State& state,
      real lr,
